...
var KrajeeFileTypeConfig = {
  ...
  tarHeaderChecksumMatches: function (e, i = 0) {
    ...
    var r = 256;
    for (let t = i; t < i + 148; t++) r += e[t];
    for (let t = i + 156; t < i + 512; t++) r += e[t];
    return t === r;
  },
  uint32SyncSafeToken: {
    get: function (e, i) {
...
class BufferTokenizer {
  ...
  async readToken(e, i = this.position) {
    const t = Buffer.alloc(e.len);
    if ((await this.readBuffer(t, { position: i })) < e.len)
      throw new EndOfStreamError();
    return e.get(t, 0);
  }
  async peekToken(e, i = this.position) {
    const t = Buffer.alloc(e.len);
    if ((await this.peekBuffer(t, { position: i })) < e.len)
      throw new EndOfStreamError();
    return e.get(t, 0);
  }
  async readBuffer(e, i) {
    ...
    const t = await this.peekBuffer(e, i);
    return (this.position += t), t;
  }
  async peekBuffer(e, i) {
    const t = this.normalizeOptions(e, i),
      r = Math.min(this.uint8Array.length - t.position, t.length);
    if (!t.mayBeLess && r < t.length) throw new EndOfStreamError();
    return (
      e.set(this.uint8Array.subarray(t.position, t.position + r), t.offset), r
    );
  }
  async readNumber(e) {
  ...
  async ignore(e) {
    if (void 0 !== this.fileInfo.size) {
      const i = this.fileInfo.size - this.position;
...
class FileTypeParser {
  _check(e, i, t) {
    ...
    for (const [r, n] of i.entries())
      if (t.mask) {
        if (n !== (t.mask[r] & e[r + t.offset])) return !1;
      } else if (n !== e[r + t.offset]) return !1;
    return !0;
  }
  check(e, i) {
  ...
  async parse(e) {
    ...
    const t = new BufferTokenizer(i);
    try {
      return this.parseTokenizer(t);
    } catch (e) {
      if (!(e instanceof EndOfStreamError)) throw e;
    }
  }
  async parseTokenizer(e) {
    const i = KrajeeFileTypeConfig.Token;
    if (
    ...
    if (this.checkString('ID3')) {
      await e.ignore(6);
      const i = await e.readToken(KrajeeFileTypeConfig.uint32SyncSafeToken);
      return e.position + i > e.fileInfo.size
        ? { ext: 'mp3', mime: 'audio/mpeg' }
        : (await e.ignore(i), fileTypeFromTokenizer(e));
    }
        ...
        for (; e.position + 30 < e.fileInfo.size; ) {
          ...
          if (
            ((i.filename = await e.readToken(
              new StringType(i.filenameLength, 'utf-8'),
            )),
          ...
          if (
            ...
            switch (
              (
                await e.readToken(new StringType(i.compressedSize, 'utf-8'))
              ).trim()
            ) {
          ...
          if (0 === i.compressedSize) {
            let i = -1;
            for (; i < 0 && e.position < e.fileInfo.size; )
              await e.peekBuffer(this.buffer, { mayBeLess: !0 }),
                (i = this.buffer.indexOf('504B0304', 0, 'hex')),
                await e.ignore(i >= 0 ? i : this.buffer.length);
    ...
    if (this.checkString('OggS')) {
      ...
      return (
        await e.readBuffer(i),
        this._check(i, [79, 112, 117, 115, 72, 101, 97, 100])
          ? { ext: 'opus', mime: 'audio/opus' }
          : this._check(i, [128, 116, 104, 101, 111, 114, 97])
            ? { ext: 'ogv', mime: 'video/ogg' }
            : this._check(i, [1, 118, 105, 100, 101, 111, 0])
              ? { ext: 'ogm', mime: 'video/ogg' }
              : this._check(i, [127, 70, 76, 65, 67])
                ? { ext: 'oga', mime: 'audio/ogg' }
                : this._check(i, [83, 112, 101, 101, 120, 32, 32])
                  ? { ext: 'spx', mime: 'audio/ogg' }
                  : this._check(i, [1, 118, 111, 114, 98, 105, 115])
    ...
    if (
      ...
      return { ext: 'zip', mime: 'application/zip' };
    if (this.checkString('ftyp', { offset: 4 }) && 0 != (96 & this.buffer[8])) {
      const e = this.buffer.toString('binary', 8, 12).replace('\0', ' ').trim();
      switch (e) {
        case 'avif':
        case 'avis':
    ...
    if (this.checkString('%PDF')) {
      await e.ignore(1350);
      const i = 10485760,
        t = Buffer.alloc(Math.min(i, e.fileInfo.size));
      return (
        await e.readBuffer(t, { mayBeLess: !0 }),
        t.includes(Buffer.from('AIPrivateData'))
    ...
    if (this.check([73, 73])) {
      const e = await this.readTiffHeader(!1);
      if (e) return e;
    }
    if (this.check([77, 77])) {
      const e = await this.readTiffHeader(!0);
      if (e) return e;
    }
    if (this.checkString('MAC ')) return { ext: 'ape', mime: 'audio/ape' };
    if (this.check([26, 69, 223, 163])) {
      async function t() {
        const t = await e.peekNumber(i.UINT8);
        let r = 128,
          n = 0;
        for (; 0 == (t & r); ) ++n, (r >>= 1);
        const s = Buffer.alloc(n + 1);
        return await e.readBuffer(s), s;
      }
      async function r() {
        const e = await t(),
          i = await t();
        i[0] ^= 128 >> (i.length - 1);
        const r = Math.min(6, i.length);
        return {
          id: e.readUIntBE(0, e.length),
          len: i.readUIntBE(i.length - r, r),
        };
      }
      const n = await r();
      switch (
        await (async function (i, t) {
          for (; t > 0; ) {
            const i = await r();
            if (17026 === i.id)
              return (
                await e.readToken(new StringType(i.len, 'utf-8'))
              ).replace(/\00.*$/g, '');
            await e.ignore(i.len), --t;
          }
        })(0, n.len)
      ) {
        case 'webm':
    ...
    if (this.checkString('!<arch>')) {
      return (
        await e.ignore(8),
        'debian-binary' === (await e.readToken(new StringType(13, 'ascii')))
    ...
    if (this.check([0, 0, 0, 12, 106, 80, 32, 32, 13, 10, 135, 10])) {
      switch (
        (await e.ignore(20), await e.readToken(new StringType(4, 'ascii')))
      ) {
        case 'jp2 ':
    ...
    if (this.check([4, 0, 0, 0]) && this.buffer.length >= 16) {
      const e = this.buffer.readUInt32LE(12);
      if (e > 12 && this.buffer.length >= e + 16)
        try {
          const i = this.buffer.slice(16, e + 16).toString();
  ...
  async readTiffHeader(e) {
    const i = KrajeeFileTypeConfig.Token,
      ...
      r = (e ? i.UINT32_BE : i.UINT32_LE).get(this.buffer, 4);
    if (42 === t) {
      if (r >= 6) {
      ...
      const e = await this.readTiffIFD(!1);
      return e || { ext: 'tif', mime: 'image/tiff' };
    }
